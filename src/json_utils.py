...
global validate_json, minify_json, largeify_json, preetify_json, write_json, convert_json, JSON_TO_DICT, DICT_TO_JSON
    ...
    if __os.path.isfile(file_name):
        if overwrite == True:
            ...
            if not indent_size or indent_size <= 0:
                ...
                with open(file_name, 'r') as O:
                    existing_json_dict = __json.load(O)
                new_json_dict = __json.loads(data)
...
def convert_json(data: any, mode: int) -> any:
    ...
    Args:
        ...
        mode: The mode (`JSON_TO_DICT`/`DICT_TO_JSON`)
